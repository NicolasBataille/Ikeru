...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("--source", type=Path, default=DEFAULT_SOURCE, help="directory holding the *.json translations")
    ...
    plans = [
        {
            "table": "vocabulary",
            # Keyed on `word`, not `id`. The ids are assigned by enumeration in
            # `generate_content_bundles.py`, so any insertion or removal in the
